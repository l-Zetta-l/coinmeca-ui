...
export default function Range(props: Slider) {
    ...
    useEffect(() => {
        if (typeof props.value === 'number' && props.value > -1)
            props.value > 100 ? setValue(100) : props.value < 0 ? setValue(0) : setValue(props.value);
    }, []);
    ...
    // const handler = (e: any, i?: any) => {
    //     if (disabled) return;
    //     const min = track.current.getBoundingClientRect().x;
    //     const max = track.current.getBoundingClientRect().width + min;
    //     const range = max - min;
    //     const point = e.clientX - min;
        ...
        <Style draggable={false}>
            ...
            <div>
                <div>
                    <div>
                        {[...Array(step)].map((_, i) => (
                            <div key={i} className={value >= (100 / (step - 1)) * i ? color : ''} />
                        ))}
                    </div>
                </div>
                <div>
